...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string word11= word1;
    string word22=word2;
    std::transform(word11.begin(), word11.end(), word11.begin(), ::toupper);
    std::transform(word22.begin(), word22.end(), word22.begin(), ::toupper);
    if(dict.count(word11)==0 || dict.count(word22)==0) return false;
    std::vector<std::string> a = dict.at(word11);
    std::vector<std::string> b = dict.at(word22);
    if(a==b) return true;
    else return false;
}
